...
async function main() {
  ...
  for (const r of rounds) {
    ...
    for (const ev of events) {
      ...
      // ALL fixtures are emitted (so the site can list the full schedule and
      // let editors fill in a result by hand), but scores are only taken from
      // the API once the match is final — never live/partial scores.
      const done = isFinished(ev);
      if (done) finished++;
      ...
      const match = {
        ...
        roundLabel,
        teamA, teamB,
        scoreA: done ? Number(ev.intHomeScore) : null,
        scoreB: done ? Number(ev.intAwayScore) : null,
        extraTime: done && (status === "AET" || status === "AP"),
        penalties: done && status === "AP",
        shootoutWinner: null,
      };

      // Apply manual override for this event, if any (this is how a result is
      // entered by hand when the API hasn't ingested it).
      const ov = OVERRIDES.byEventId && OVERRIDES.byEventId[match.eventId];
      if (ov) Object.assign(match, ov);

      const hasResult = Number.isFinite(match.scoreA) && Number.isFinite(match.scoreB);
      ...
      if (hasResult && match.stage === "knockout" && match.scoreA === match.scoreB) {
      ...
      if (hasResult && match.stage === "knockout" && match.scoreA === match.scoreB && !match.shootoutWinner) {
        warnings.needsReview.push({
          ...
          label: `${teamA} ${match.scoreA}–${match.scoreB} ${teamB} (${roundLabel}, ${match.date})`,
        });
      }
  ...
  /* ---- merge in manual matches (data/overrides.js is their single source of
     truth — the copies baked into data/matches.js are just generated output, so
     a match removed from overrides disappears on the next run) ---- */
  let existing = [];
  ...
  // so the real API result takes over once it ingests a manually entered game.
  const apiKeys = new Set(apiMatches.map(keyOf));
  const manual = manualFromOverrides.filter((m) => !apiKeys.has(keyOf(m)));
